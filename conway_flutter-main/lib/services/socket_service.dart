...
class SocketService {
  ...
  void connect(String userId) {
    ...
    if (_userId == userId && isConnected) {
      print("[SocketService] Already connected for user $userId. Re-emitting join event.");
      _socket!.emit('join', _userId); // Re-emit join just in case state is inconsistent
      return;
    }

    // If switching users or connecting for the first time/after disconnect
    print("[SocketService] Attempting to connect/reconnect for user: $userId (Previous userId: $_userId)");
  ...
  void _setupListeners() {
    ...
    _socket!.onConnect((_) {
      ...
      if (_userId != null && _userId!.isNotEmpty) {
        print('[SocketService ON connect] Emitting join event for userId: $_userId');
        _socket!.emit('join', _userId); 
      } else {
        print('[SocketService ON connect] Warning: Cannot emit join, userId is null or empty.');
      }
    });

    _socket!.onConnectError((data) => print('[SocketService ON connect_error] Error: $data'));
    ...
    _socket!.on('receiveMessage', (data) {
      ...
      if (data is Map<String, dynamic>) {
         print('[SocketService ON receiveMessage] Pushing data to stream controller...');
         _messageController.add(data);
      } else {
         print('[SocketService ON receiveMessage] Received invalid message format');
      }
    });
    ...
    _socket!.on('messageSent', (data) {
        print('[SocketService ON messageSent] Delivery confirmed: $data');
    });
 }

  void sendMessage(String senderId, String senderEmail, String receiverEmail, String plainMessageText) {
    if (isConnected) {
      // ... (sending logic as before) ...
       print('[SocketService] Sending plain message via socket:');
       // ... logs ...
  ...
  void disconnect() {
    print("[SocketService] disconnect() called. Disposing socket for userId: $_userId");
